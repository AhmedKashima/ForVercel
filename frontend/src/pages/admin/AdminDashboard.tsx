...
const AdminDashboard = ({ token }: { token: string }) => {
  ...
  const openActiveProjects = async () => {
    ...
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/active-projects`, {
    ...
    const fetchData = async () => {
      try {
        const [metricsRes, activityRes, revenueRes] = await Promise.all([
          axios.get(`${import.meta.env.VITE_API_URL}/api/admin/metrics`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get(`${import.meta.env.VITE_API_URL}/api/admin/activity?days=14`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get(`${import.meta.env.VITE_API_URL}/api/dashboard/revenue_over_time`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
